...
# make a table of relevant pvalues
...
write.csv(tab2.ulgr, "output/steelhead_ulgr_table2.csv", row.names=F, quote=F)


# --- Get CI's for odds ratios on length and nbypass

# zlength
extractCI(modsums.ulgr[[8]], "zlength")

# nbypass
extractCI(modsums.ulgr[[8]], "nbypass")
...
write.csv(tab2.lgr, "output/steelhead_lgr_table2.csv", row.names=F, quote=F)


# --- Get CI's for odds ratios on length and nbypass

# zlength
extractCI(modsums.lgr[[8]], "zlength")

# nbypass
extractCI(modsums.lgr[[8]], "nbypass")
...
tmpdat.ulgr <- sdat.ulgr

for (k in 1:nsim) {
  set.seed(k*17) 
  cat("sim ", k, " of", nsim, "\n")
  tmp.pars <- rmvnorm(n=1, mean=c(bmu.ulgr.m0, rmu.ulgr.m0), sigma=VC.ulgr.m0 )
  tmp.pred.eta <- X.ulgr.m0%*%as(as.matrix(tmp.pars[1:6]), Class="Matrix") + Z.ulgr.m0%*%as(as.matrix(tmp.pars[7:17]), Class="Matrix")
  ...
  write.csv(simout.ulgr.m0, file="output/sthd_ulgr_dataM0_simulation_outputs.csv", quote=FALSE, row.names=FALSE)
  if (k==nsim) cat("Done!", "\n")
}
...
tab3.ulgr.m0 <- data.frame(data.model=rep("M0", 3), fit.model=rep("M1",3), 
                    ...
                    steelhead.ULGD=c(p.n.ulg.m0.m1, p.ns.ulg.m0.m1, p.na.ulg.m0.m1) )
...
for (k in 1:nsim) {
  ...
  tmp.pred.eta <- X.ulgr.m3%*%as(as.matrix(tmp.pars[1:7]), Class="Matrix") + Z.ulgr.m3%*%as(as.matrix(tmp.pars[8:18]), Class="Matrix")
  tmp.pred <- plogis(as.vector(as(tmp.pred.eta, Class="matrix")))
  # generate new adult return data
  tmp.y <- rbinom(n=length(tmp.pred.eta), size=1, prob=tmp.pred)
  tmpdat.ulgr$adult_return <- tmp.y
  ...
  write.csv(simout.ulgr.m3, file="output/sthd_ulgr_dataM3_simulation_outputs.csv", quote=FALSE, row.names=FALSE)
  if (k==nsim) cat("Done!", "\n")
}
...
tab3.ulgr.m3 <- data.frame(data.model=rep("M3", 6), fit.model=rep(c("M1","M2"),rep(3,2)), 
                    ...
                    steelhead.ULGD=c(p.n.ulg.m3.m1, p.ns.ulg.m3.m1, p.na.ulg.m3.m1, p.n.ulg.m3.m2, p.ns.ulg.m3.m2, p.na.ulg.m3.m2  ) )
...
formula.lgr.M0 <- formula(paste("adult_return ~ ", covsets.lgr2[[bestmod.cov.lgr]] ))
...
for (k in 1:nsim) {
  ...
  write.csv(simout.lgr.m0, file="output/sthd_lgr_dataM0_simulation_outputs.csv", quote=FALSE, row.names=FALSE)
  if (k==nsim) cat("Done!", "\n")
}
...
tab3.lgr.m0 <- data.frame(data.model=rep("M0", 3), fit.model=rep("M1",3), 
                    outcome=outcome.labs, expected=expected.labs,
                    steelhead.LGD=c(p.n.lg.m0.m1,p.ns.lg.m0.m1,p.na.lg.m0.m1) )
...
for (k in 1:nsim) {
  ...
  write.csv(simout.lgr.m3, file="output/sthd_lgr_dataM3_simulation_outputs.csv", quote=FALSE, row.names=FALSE)
  if (k==nsim) cat("Done!", "\n")
}
...
tab3.lgr.m3 <- data.frame(data.model=rep("M3", 6), fit.model=rep(c("M1","M2"),rep(3,2)), 
                    outcome=rep(outcome.labs,2), expected=rep(expected.labs,2),
                    steelhead.LGD=c(p.n.lg.m3.m1,p.ns.lg.m3.m1,p.na.lg.m3.m1, p.n.lg.m3.m2,p.ns.lg.m3.m2,p.na.lg.m3.m2  ) )
...
write.csv(tab3, file="output/steelhead_simulations_table3.csv", quote=F, row.names=F)
