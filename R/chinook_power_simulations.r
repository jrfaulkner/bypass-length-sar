...
### R Code for calculating power to detect bypass effects for Chinook at
...
fcdat.ulgr = read.csv("data/chinook_ULGR_SAR_data.csv", stringsAsFactors=F) 
fcdat.ulgr$fyear = factor(fcdat.ulgr$year)

yrset = 2004:2014
nyear = length(yrset)

# Table for number of bypass events by year
byptab.ulgr = table(fcdat.ulgr$year, fcdat.ulgr$nbypass)
# Create data frame from table
cdat1.ulgr = as.data.frame(byptab.ulgr)
names(cdat1.ulgr) = c("year", "nbypass", "nfish")
cdat1.ulgr = cdat1.ulgr[order(cdat1.ulgr$year, cdat1.ulgr$nbypass), ]


## -- Tagged at LGR -----------
# data used for SAR model fitting
fcdat.lgr = read.csv("data/chinook_LGR_SAR_data.csv", stringsAsFactors=F) 
fcdat.lgr$fyear = factor(fcdat.lgr$year)


# Table for number of bypass events by year
byptab.lgr = table(fcdat.lgr$year, fcdat.lgr$nbypass)
# Create data frame from table
cdat1.lgr = as.data.frame(byptab.lgr)
names(cdat1.lgr) = c("year", "nbypass", "nfish")
cdat1.lgr = cdat1.lgr[order(cdat1.lgr$year, cdat1.lgr$nbypass), ]
...
tfit.ulgr = glmer(adult_return ~ nbypass + (1 | fyear), data=fcdat.ulgr, family=binomial)
...
tfit.lgr = glmer(adult_return ~ nbypass + (1 | fyear), data=fcdat.lgr, family=binomial)
# extract random year effects
...
tmp.nbyp.css1 = -0.117  # 2010 value
tmp.nbyp.css2 = -0.125  # 2016 value

## Calculate return probabilities
cdat1.ulgr$nu.alt1 = tmp.int.ulgr + tmp.yreff.ulgr + tmp.nbyp.alt1*cdat1.ulgr$nbypass
cdat1.ulgr$psar.alt1 = plogis(cdat1.ulgr$nu.alt1)
cdat1.ulgr$nu.alt2 = tmp.int.ulgr + tmp.yreff.ulgr + tmp.nbyp.alt2*cdat1.ulgr$nbypass
cdat1.ulgr$psar.alt2 = plogis(cdat1.ulgr$nu.alt2)
cdat1.ulgr$nu.css1 = tmp.int.ulgr + tmp.yreff.ulgr + tmp.nbyp.css1*cdat1.ulgr$nbypass
cdat1.ulgr$psar.css1 = plogis(cdat1.ulgr$nu.css1)
cdat1.ulgr$nu.css2 = tmp.int.ulgr + tmp.yreff.ulgr + tmp.nbyp.css2*cdat1.ulgr$nbypass
cdat1.ulgr$psar.css2 = plogis(cdat1.ulgr$nu.css2)
## drop groupings with zero fish
cdat.ulgr = cdat1.ulgr[cdat1.ulgr$nfish!=0, ]
cdat.ulgr$fyear = as.character(cdat.ulgr$year)

nobs.ulgr = nrow(cdat.ulgr) #number of observation groupings
...
for (k in 1:nsim) {
  ...
  set.seed(k*10)
  tmp.return.css1 = rbinom(nobs.ulgr, size=cdat.ulgr$nfish, prob=cdat.ulgr$psar.css1)
  tmp.noreturn.css1 = as.integer(cdat.ulgr$nfish - tmp.return.css1)
  tmp.adult_return.css1 = cbind(tmp.return.css1, tmp.noreturn.css1)
  # fit models with and without nbypass
  tmp.fit.css1 = glmer(tmp.adult_return.css1 ~ nbypass + (1 | fyear), data=cdat.ulgr, family=binomial)
  tmp.fit.css1.yr = glmer(tmp.adult_return.css1 ~ (1 | fyear), data=cdat.ulgr, family=binomial)
  ...
  # generate random number of returning fish
  set.seed(k*10)
  tmp.return.css2 = rbinom(nobs.ulgr, size=cdat.ulgr$nfish, prob=cdat.ulgr$psar.css2)
  tmp.noreturn.css2 = as.integer(cdat.ulgr$nfish - tmp.return.css2)
  tmp.adult_return.css2 = cbind(tmp.return.css2, tmp.noreturn.css2)
  # fit models with and without nbypass
  tmp.fit.css2 = glmer(tmp.adult_return.css2 ~ nbypass + (1 | fyear), data=cdat.ulgr, family=binomial)
  tmp.fit.css2.yr = glmer(tmp.adult_return.css2 ~ (1 | fyear), data=cdat.ulgr, family=binomial)
  ...
  # generate random number of returning fish
  set.seed(k*10)
  tmp.return.alt1 = rbinom(nobs.ulgr, size=cdat.ulgr$nfish, prob=cdat.ulgr$psar.alt1)
  tmp.noreturn.alt1 = as.integer(cdat.ulgr$nfish - tmp.return.alt1)
  tmp.adult_return.alt1 = cbind(tmp.return.alt1, tmp.noreturn.alt1)
  # fit models with and without nbypass
  tmp.fit.alt1 = glmer(tmp.adult_return.alt1 ~ nbypass + (1 | fyear), data=cdat.ulgr, family=binomial)
  tmp.fit.alt1.yr = glmer(tmp.adult_return.alt1 ~ (1 | fyear), data=cdat.ulgr, family=binomial)
  ...
  # generate random number of returning fish
  set.seed(k*10)
  tmp.return.alt2 = rbinom(nobs.ulgr, size=cdat.ulgr$nfish, prob=cdat.ulgr$psar.alt2)
  tmp.noreturn.alt2 = as.integer(cdat.ulgr$nfish - tmp.return.alt2)
  tmp.adult_return.alt2 = cbind(tmp.return.alt2, tmp.noreturn.alt2)
  # fit models with and without nbypass
  tmp.fit.alt2 = glmer(tmp.adult_return.alt2 ~ nbypass + (1 | fyear), data=cdat.ulgr, family=binomial)
  tmp.fit.alt2.yr = glmer(tmp.adult_return.alt2 ~  (1 | fyear), data=cdat.ulgr, family=binomial)
...
## Table for results
simsum.ulgr = data.frame(species="Chinook", tagsite="ULGR", matrix(0,4,5))
...
for (k in 1:nsim) {
  ...
  set.seed(k*10)
  tmp.return.css = rbinom(nobs.ulgr, size=cdat.ulgr$nfish, prob=cdat.ulgr$psar.css1)
  tmp.noreturn.css = as.integer(cdat.ulgr$nfish - tmp.return.css)
  tmp.adult_return.css = cbind(tmp.return.css, tmp.noreturn.css)

  tmp.fit.css = glm(tmp.adult_return.css ~ nbypass + fyear, data=cdat.ulgr, family=binomial)
...
## Calculate return probabilities
cdat1.lgr$nu.alt1 = tmp.int.lgr + tmp.yreff.lgr + tmp.nbyp.alt1*cdat1.lgr$nbypass
cdat1.lgr$psar.alt1 = plogis(cdat1.lgr$nu.alt1)
cdat1.lgr$nu.alt2 = tmp.int.lgr + tmp.yreff.lgr + tmp.nbyp.alt2*cdat1.lgr$nbypass
cdat1.lgr$psar.alt2 = plogis(cdat1.lgr$nu.alt2)
cdat1.lgr$nu.css1 = tmp.int.lgr + tmp.yreff.lgr + tmp.nbyp.css1*cdat1.lgr$nbypass
cdat1.lgr$psar.css1 = plogis(cdat1.lgr$nu.css1)
cdat1.lgr$nu.css2 = tmp.int.lgr + tmp.yreff.lgr + tmp.nbyp.css2*cdat1.lgr$nbypass
cdat1.lgr$psar.css2 = plogis(cdat1.lgr$nu.css2)

## drop groupings with zero fish
cdat.lgr = cdat1.lgr[cdat1.lgr$nfish!=0, ]
cdat.lgr$fyear = as.character(cdat.lgr$year)

nobs.lgr = nrow(cdat.lgr)  #number of observation groupings
...
for (k in 1:nsim) {
  ...
  # generate random number of returning fish
  set.seed(k*10)
  tmp.return.css1 = rbinom(nobs.lgr, size=cdat.lgr$nfish, prob=cdat.lgr$psar.css1)
  tmp.noreturn.css1 = as.integer(cdat.lgr$nfish - tmp.return.css1)
  tmp.adult_return.css1 = cbind(tmp.return.css1, tmp.noreturn.css1)
  # fit models with and without nbypass
  tmp.fit.css1 = glmer(tmp.adult_return.css1 ~ nbypass + (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.fit.css1.yr = glmer(tmp.adult_return.css1 ~ (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.sum.css1 = summary(tmp.fit.css1)
  # collect outputs
  ...
  # generate random number of returning fish
  set.seed(k*10)
  tmp.return.css2 = rbinom(nobs.lgr, size=cdat.lgr$nfish, prob=cdat.lgr$psar.css2)
  tmp.noreturn.css2 = as.integer(cdat.lgr$nfish - tmp.return.css2)
  tmp.adult_return.css2 = cbind(tmp.return.css2, tmp.noreturn.css2)
  # fit models with and without nbypass
  tmp.fit.css2 = glmer(tmp.adult_return.css2 ~ nbypass + (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.fit.css2.yr = glmer(tmp.adult_return.css2 ~ (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.sum.css2 = summary(tmp.fit.css2)
  # collect outputs
  ...
  # generate random number of returning fish
  set.seed(k*10)
  tmp.return.alt1 = rbinom(nobs.lgr, size=cdat.lgr$nfish, prob=cdat.lgr$psar.alt1)
  tmp.noreturn.alt1 = as.integer(cdat.lgr$nfish - tmp.return.alt1)
  tmp.adult_return.alt1 = cbind(tmp.return.alt1, tmp.noreturn.alt1)
  # fit models with and without nbypass
  tmp.fit.alt1 = glmer(tmp.adult_return.alt1 ~ nbypass + (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.fit.alt1.yr = glmer(tmp.adult_return.alt1 ~ (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.sum.alt1 = summary(tmp.fit.alt1)
  # collect outputs
  ...
  # generate random number of returning fish
  set.seed(k*10)
  tmp.return.alt2 = rbinom(nobs.lgr, size=cdat.lgr$nfish, prob=cdat.lgr$psar.alt2)
  tmp.noreturn.alt2 = as.integer(cdat.lgr$nfish - tmp.return.alt2)
  tmp.adult_return.alt2 = cbind(tmp.return.alt2, tmp.noreturn.alt2)
  # fit models with and without nbypass
  tmp.fit.alt2 = glmer(tmp.adult_return.alt2 ~ nbypass + (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.fit.alt2.yr = glmer(tmp.adult_return.alt2 ~  (1 | fyear), data=cdat.lgr, family=binomial)
  tmp.sum.alt2 = summary(tmp.fit.alt2)
  # collect outputs
...
## Table for results
simsum.lgr = data.frame(species="Chinook", tagsite="LGR", matrix(0,4,5))
...
write.csv(simsum.out, file="output/chinook_powersim_summary_table.csv", row.names=F, quote=F)
...
for (k in 1:nsim) {
  ...
  set.seed(k*10)
  tmp.return.css = rbinom(nobs.lgr, size=cdat.lgr$nfish, prob=cdat.lgr$psar.css1)
  tmp.noreturn.css = as.integer(cdat.lgr$nfish - tmp.return.css)
  tmp.adult_return.css = cbind(tmp.return.css, tmp.noreturn.css)

  tmp.fit.css = glm(tmp.adult_return.css ~ nbypass + fyear, data=cdat.lgr, family=binomial)
  tmp.sum.css = summary(tmp.fit.css)
...
for (j in nmult) {
  ...
  for (k in 1:nsim) {
    ...
    set.seed(k*9)
    tmp.cdat1.ulgr = data.frame(year=as.character(rep(yrset, rep(8,nyear))), nbypass=rep(0:7,nyear), nfish=0)
    ...
    tmp.cdat1.ulgr$nfish = tmp.nfv.ulgr
    tmp.cdat1.ulgr$nu.css1 = tmp.int.ulgr + tmp.yreff.ulgr + tmp.nbyp.css1*tmp.cdat1.ulgr$nbypass
    tmp.cdat1.ulgr$psar.css1 = plogis(tmp.cdat1.ulgr$nu.css1)
    tmp.cdat.ulgr = tmp.cdat1.ulgr[tmp.cdat1.ulgr$nfish!=0, ]
    tmp.cdat.ulgr$fyear = as.character(tmp.cdat.ulgr$year)
    tmp.nobs.ulgr = nrow(tmp.cdat.ulgr)
    # Generate number of fish returning  
    set.seed(k*10)
    tmp.return.css1 = rbinom(tmp.nobs.ulgr, size=tmp.cdat.ulgr$nfish, prob=tmp.cdat.ulgr$psar.css1)
    tmp.noreturn.css1 = as.integer(tmp.cdat.ulgr$nfish - tmp.return.css1)
    tmp.adult_return.css1 = cbind(tmp.return.css1, tmp.noreturn.css1)
    # Fit model and record pvalue for nbypass
    tmp.fit.css1 = glmer(tmp.adult_return.css1 ~ nbypass + (1 | fyear), data=tmp.cdat.ulgr, family=binomial)
    tmp.sum.css1 = summary(tmp.fit.css1)
...
write.csv(data.frame(size=1:9, power=npowvec.ulgr), "output/chinook_power_by_samplesize_summary.csv", quote=F, row.names=F)
